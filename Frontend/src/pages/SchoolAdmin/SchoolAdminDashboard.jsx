...
const SchoolAdminDashboard = () => {
    ...
    const navItems = [
        ...
        { id: 'student', label: 'Student Management', icon: '👩‍🎓' },
        { id: 'teacher', label: 'Teacher Management', icon: '👨‍🏫' },
        { id: 'principal', label: 'Principal Details', icon: '🏫' },
        { id: 'parent', label: 'Parent Management', icon: '👨‍👩‍👧' },
    ...
    const renderContent = () => {
        switch (activeTab) {
            ...
            case 'principal': return <PrincipalDetails />;
            case 'parent': return <ParentManagement />;
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: 'linear-gradient(135deg, #f1f5f9 0%, #e2e8f0 100%)', fontFamily: "'Inter', sans-serif" }}>
            ...
            <aside style={{
                ...
                <div style={{ padding: '24px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 40, height: 40, background: 'linear-gradient(135deg, #f97316, #fb923c)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, boxShadow: '0 4px 12px rgba(249,115,22,0.4)' }}>🏫</div>
                    <div style={{ flex: 1, overflow: 'hidden' }}>
                        ...
                        <p style={{ color: '#fb923c', margin: 0, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>School Admin Portal</p>
                    </div>
                </div>
                        ...
                        {navItems.map(item => (
                            <button
                                ...
                                style={{
                                    ...
                                    background: activeTab === item.id ? 'linear-gradient(90deg, rgba(249,115,22,0.15), transparent)' : 'transparent',
                                    color: activeTab === item.id ? '#fdba74' : '#cbd5e1',
                                    transition: 'all 0.2s ease',
                                    boxShadow: activeTab === item.id ? 'inset 3px 0 0 #f97316' : 'none'
                                }}
                ...
                <div style={{ padding: '20px 16px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 12, marginBottom: 12 }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#f97316', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>
                            {currentUser?.name ? currentUser.name.charAt(0) : 'A'}
                        </div>
                        <div style={{ overflow: 'hidden' }}>
                            <p style={{ color: 'white', margin: 0, fontSize: 14, fontWeight: 600, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{currentUser?.name || 'Admin User'}</p>
                            <p style={{ color: '#94a3b8', margin: 0, fontSize: 11 }}>School Administrator</p>
                        </div>
                    </div>

...
const ReceptionistDashboard = () => {
    ...
    const renderContent = () => {
        switch (activeTab) {
            case 'overview': return <ReceptionistOverview />;
            case 'visitors': return <PlaceholderView title="Visitor Log" />;
            case 'enquiries': return <PlaceholderView title="Admission Enquiries" />;
            case 'notices': return <PlaceholderView title="Front Desk Notices" />;
            default: return <ReceptionistOverview />;
        }
    };

    const getActiveTitle = () => {
        const item = navItems.find(n => n.id === activeTab);
        return item ? item.label : 'Front Desk Overview';
    };
        ...
        <div style={{ display: 'flex', minHeight: '100vh', background: 'linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%)', fontFamily: "'Inter', sans-serif" }}>
            ...
            <aside style={{
                ...
                <div style={{ padding: '24px 20px', borderBottom: '1px solid rgba(255,255,255,0.05)', display: 'flex', alignItems: 'center', gap: 12 }}>
                    <div style={{ width: 40, height: 40, background: 'linear-gradient(135deg, #ec4899, #db2777)', borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, boxShadow: '0 4px 12px rgba(236,72,153,0.4)' }}>🛎️</div>
                    <div style={{ flex: 1, overflow: 'hidden' }}>
                        <h2 style={{ color: 'white', margin: 0, fontSize: 16, fontWeight: 800, whiteSpace: 'nowrap', textOverflow: 'ellipsis', overflow: 'hidden' }}>
                            VidyaSetu
                        </h2>
                        <p style={{ color: '#f472b6', margin: 0, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Reception Portal</p>
                    </div>
                </div>
                        ...
                        {navItems.map(item => (
                            <button
                                ...
                                style={{
                                    ...
                                    background: activeTab === item.id ? 'linear-gradient(90deg, rgba(236,72,153,0.15), transparent)' : 'transparent',
                                    color: activeTab === item.id ? '#f472b6' : '#94a3b8',
                                    transition: 'all 0.2s ease',
                                    boxShadow: activeTab === item.id ? 'inset 3px 0 0 #ec4899' : 'none'
                                }}
                ...
                <div style={{ padding: '20px 16px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', background: 'rgba(255,255,255,0.03)', borderRadius: 12, marginBottom: 12 }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: '#ec4899', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 'bold' }}>
                            {currentUser?.name ? currentUser.name.charAt(0).toUpperCase() : 'R'}
                        </div>
                        <div style={{ overflow: 'hidden' }}>
                            <p style={{ color: 'white', margin: 0, fontSize: 14, fontWeight: 600, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>{currentUser?.name || 'Receptionist'}</p>
                            <p style={{ color: '#94a3b8', margin: 0, fontSize: 11 }}>Front Desk</p>
                        </div>
                    </div>

...
const SchoolRegister = () => {
  ...
  const isSuperAdmin = currentRole === 'Super Admin' || !currentRole;

  const [userRole,      setUserRole]      = useState(isSuperAdmin ? 'School Admin' : 'Teacher');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage(`✅ ${userRole} registered successfully!`);
        setMessageType('success');
        setTimeout(() => navigate('/SuperAdmin'), 1800);
      } else {
        setMessage(data.message || `Error registering ${userRole}`);
        ...
        <form onSubmit={handleSubmit} className="px-8 py-7 space-y-8">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className={labelCls}>{isSuperAdmin ? 'Admin Name *' : 'Name *'}</label>
                <input required type="text" name="name" value={formData.name} onChange={handleChange} className={inputCls} placeholder="Full Name" />
              </div>
              <div>
                <label className={labelCls}>{isSuperAdmin ? 'Admin Email (Login ID) *' : 'Email (Login ID) *'}</label>
                <input required type="email" name="email" value={formData.email} onChange={handleChange} className={inputCls} placeholder="email@example.com" />
              </div>
              <div>
                <label className={labelCls}>{isSuperAdmin ? 'Admin Phone' : 'Phone'}</label>
              ...
              {!isSuperAdmin && userRole === 'Student' && (
                <>
                  <div>
                    ...
                    <input required type="number" name="classId" value={formData.classId} onChange={handleChange} className={inputCls} placeholder="Class ID" />
                  </div>
                  <div>
                    <label className={labelCls}>Admission No *</label>
                    <input required type="text" name="admissionNo" value={formData.admissionNo} onChange={handleChange} className={inputCls} placeholder="Admission No" />
                  </div>
                </>
              )}
            </div>
            ...
            >
              {loading ? (
                ...
                <>
                  ...
                  {isSuperAdmin ? 'Create School & Admin Account' : `Create ${userRole} Account`}
                </>
              )}
            </button>
          </div>
